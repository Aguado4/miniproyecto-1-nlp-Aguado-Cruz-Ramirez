...
> El EDA del notebook debe reproducirlas; si no coinciden, hay un bug en el EDA.
...
- **EDA:** corpus completo, 208,051 registros.
- **Modelado:** submuestra estratificada por `Polarity` de **40,000** registros, `SEED = 42`,
  preservando las proporciones de §3.1. Justificación en `DECISIONS.md` §D-004.
- **Split A (aleatorio):** 80/10/10 estratificado por `Polarity`.
- **Split B (geográfico):** regiones held-out para test; el conjunto exacto se fija en el
  notebook y se documenta aquí cuando se decida.
